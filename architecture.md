...
CREATE TABLE listing_items (
  ...
  status      VARCHAR(16) NOT NULL DEFAULT 'AVAILABLE', -- AVAILABLE | SOLD
  sold_at     TIMESTAMPTZ,
  UNIQUE (listing_id, item_index),          -- 同一 listing 内 item_index 唯一
  CHECK (status IN ('AVAILABLE', 'SOLD'))   -- COUNT(status='AVAILABLE') 强依赖此字符串精确无误
);
...
CREATE TABLE tags (
  ...
  name        VARCHAR(32) UNIQUE NOT NULL,  -- UNIQUE 自带索引，服务等值查询；前缀补全见下方说明
  usage_count INTEGER NOT NULL DEFAULT 0    -- 热门标签排序依据，应用层维护
);
-- 前缀自动补全（GET /tags?prefix=）注意：默认 collation 下 btree 不支持 LIKE 'x%' 走索引。
-- 校园量级 tag 数量预计仅几百，seq scan 完全可接受；若日后需要，可加
--   CREATE INDEX idx_tags_name_pattern ON tags (name varchar_pattern_ops);
-- usage_count 口径与维护策略（须统一，否则两套逻辑收敛到不同值）：
--   口径定义：usage_count = "该 tag 当前关联的 ACTIVE listing 数"（与广场展示语义一致）。
--   1. 应用层：在以下事务中同步 ±1
--      a) listing 创建（ACTIVE）/ 增删 tag      → ±1
--      b) listing 状态 ACTIVE ↔ 非 ACTIVE 切换  → ±N（N = 该 listing 的 tag 数）
--   2. 兜底：每日定时任务全量重算，必须与口径一致：
--      UPDATE tags SET usage_count = (
--        SELECT COUNT(*) FROM listing_tags lt
--        JOIN listings l ON l.id = lt.listing_id
--        WHERE lt.tag_id = tags.id AND l.status = 'ACTIVE'
--      )
--   避免触发器对 ORM 不透明的问题。
...
CREATE TABLE wishes (
  ...
  keyword      VARCHAR(255) NOT NULL,       -- 用于匹配新上架商品的关键词；POST /wishes 未提供时应用层默认取 title
  max_price    INTEGER,                     -- 期望最高价（单位：分，与 listings.price_cents 一致），NULL = 不限
...
-- 求购广场列表 + 匹配扫描（WHERE status='ACTIVE'）：前导列即 status，单一索引即可服务两类查询
...
**召回与排序注意点**：
- `similarity(a, b) = |trigram 交集| / |trigram 并集|`。当查询很短（如 2 字 CJK "数学"）而文档很长时，并集被长文本撑大，`similarity` 会被稀释到阈值（默认 0.3）以下而漏召。**对短查询应使用 `word_similarity` 或操作符 `%>`（"文档中存在与查询近似的子串"）**，它在文档上滑窗取最佳子串匹配，关键词整体命中时 ≈1.0。`gin_trgm_ops` 同样支持 `%>`/`<%`，索引照样可用：
  ```sql
  WHERE search_text %> :query        -- 替代 search_text % :query
  ORDER BY word_similarity(:query, search_text) DESC
  ```
- GIN trgm 索引只服务 `%`/`%>` 的过滤，`ORDER BY similarity DESC` 仍需排序步骤（要走索引排序得用 GiST + `<->`）。`LIMIT 20` 量小可接受。

**游标分页 tiebreaker**：`created_at DESC` 在并列时间戳上必须以 `id` 兜底，否则翻页可能漏行/重复。统一约定游标编码 `(排序键, id)`，SQL 形如 `ORDER BY created_at DESC, id DESC`，相应索引可考虑改为 `(status, created_at DESC, id DESC)`（校园量级即便不改也可接受）。
...
**Exposed 实现注意**：`listings.search_text` / `wishes.search_text` 是 PostgreSQL `GENERATED ALWAYS AS ... STORED` 生成列，向其 INSERT/UPDATE 会被数据库拒绝（"cannot insert into column ... GENERATED ALWAYS"）。在 Exposed 的 Table 定义里把这两列、`id`（gen_random_uuid 默认）、`created_at`（NOW 默认）等设为 `databaseGenerated()` / 只读，不要纳入 insert 列集合。
...
| `POST /auth/register` | body: `studentId, bitPassword, platformPassword, nickname, qq?, wechat?, phone?` → 调 BIT101 校验 → 写库 → 返回 `{accessToken, refreshToken}`。**应用层校验 qq/wechat/phone 至少一项非空**（无联系方式无法被联系），客户端推荐 QQ/微信、对 phone 给隐私风险提示（见 §4.10） |
| `POST /auth/login` | body: `studentId, platformPassword` |
| `POST /auth/refresh` | |
| `POST /auth/logout` | 撤销 refresh token |
| `GET /users/me`, `PATCH /users/me` | PATCH 修改 qq/wechat/phone 时若三者最终全为空 → 422（卖家/买家无法被联系） |
...
| `POST /listings` | 创建出售单（含子项目数量、可选 expires_at，默认 6 个月）。**应用层前置校验**：当前用户 qq/wechat/phone 至少一项非空，否则 422 并引导补全 profile |
...
| `PATCH /listings/{id}` | |
| `POST /listings/{id}/items/{idx}/sold` | 标记已售一件。**事务开头先 `SELECT id FROM listings WHERE id=:id FOR UPDATE` 锁父行**，把同一 listing 的所有 sold/unsell 串行化，再更新 item、再 `COUNT(status='AVAILABLE')==0` 时把 listing 置 SOLD_OUT。仅锁 item 行无法防住 SOLD_OUT 跨 item 的并发丢失竞态 |
| `POST /listings/{id}/items/{idx}/unsell` | **取消售出**：SOLD → AVAILABLE；同一事务先锁父行，若 listing 当前为 SOLD_OUT 则同步改回 ACTIVE 并清空 sold_at。供卖家误标后恢复，仅本人可调用 |
...
| `GET /wishes/{id}` | 求购详情。**求购者联系方式仅当 status=ACTIVE 且请求者已登录时返回**，FULFILLED/EXPIRED 不返回（与 listings 对称，呼应"找到后免打扰"） |
| `POST /wishes` | 发布求购（title 必填；keyword 缺省取 title；可选 description/max_price/tag_filter/expires_at）。**应用层前置校验**：当前用户 qq/wechat/phone 至少一项非空，否则 422 |
...
- **可浏览的求购帖**：`GET /wishes` 是求购广场，列出全站活跃求购（pg_trgm 模糊搜索 + 标签/价格筛选）；`GET /wishes/{id}` 详情在 status=ACTIVE 且请求者已登录时展示求购者联系方式（QQ/微信/手机，取自 users 表），卖家可据此主动联系买家。与 listings 对称，不在平台内成交。
- **关键词订阅（自动匹配通知）**：新 listing 创建后**异步**（提交事务后投递到协程队列，不阻塞创建接口）扫描全部 ACTIVE wish 做匹配：
  ```sql
  -- 用 word_similarity / %> 而非 similarity，避免短关键词("CSAPP")比长 search_text
  -- 时相似度被并集稀释、几乎永不命中的语义 bug。
  -- 此处用 listing.search_text 作"文档"、wish.keyword 作"查询"，匹配后再叠加价格/标签过滤。
  SELECT w.* FROM wishes w
  WHERE w.status = 'ACTIVE'
    AND :listing_search_text %> w.keyword       -- 即 word_similarity(w.keyword, :listing_search_text) >= 阈值
    AND (w.max_price IS NULL OR :listing_price_cents IS NULL OR :listing_price_cents <= w.max_price)
    AND (w.tag_filter IS NULL OR w.tag_filter && :listing_tag_ids);
  ```
  命中则写一条 `WISH_HIT` 通知。校园量级下活跃求购有限，逐条扫描可接受；数据量大后可反向对 `wish.keyword` 建 trgm 索引、用 listing 文本反查。
  **通知落地体验**：WISH_HIT 通知可能在卖家把商品标记 SOLD_OUT 之后才被买家看到，落地页须明确提示"该商品已售出"，避免买家在已售商品上反复尝试联系。
- **免打扰**：求购者找到后调 `POST /wishes/{id}/fulfill`（status → FULFILLED），即从广场与匹配池中移除；详情接口同步不再返回求购者联系方式。

### 4.10 联系方式策略

需求 2"方便联系卖家或买家"是产品存在的理由，必须有强制保证：

**字段与必填**：`users.qq / wechat / phone` 均可空，但**应用层在以下三处强制"至少一项非空"**——
- `POST /auth/register`：注册即必须留下至少一种联系方式。
- `PATCH /users/me`：编辑后若三者全空则 422。
- `POST /listings` 与 `POST /wishes`：发布前置校验，否则 422 并引导客户端跳转 profile 补全。

**手机号风险提示**（客户端 UX 层）：注册/编辑联系方式表单中，对 `phone` 字段下方常驻一条警示：
> ⚠️ 手机号会暴露给所有登录用户。校园场景下推荐使用 **QQ / 微信** 作为主要联系方式，仅在你愿意接受陌生号码来电/短信时填写手机号。

设置页"我的联系方式"同样展示该提示。注册流程默认聚焦 QQ/微信字段，phone 折叠在"更多联系方式"下避免无意识勾选。

**可见性**：登录后才返回联系方式；listings/wishes 详情仅当 status=ACTIVE 时返回（售出/下架/过期/fulfill 后即停止暴露，呼应"免打扰"）。

**未填空态**：`GET /listings/{id}` 与 `GET /wishes/{id}` 的 DTO 必须能表达"卖家/买家未留某种联系方式"，客户端据此隐藏对应入口；若三者全空（极端情况，如老数据），详情页显示"该用户未留下联系方式"——但因有上述前置校验，新数据不会出现此态。
...
**前置门槛**：进入发布流时，先用本地缓存的用户 profile 检查 qq/wechat/phone 是否至少一项非空；若全空，弹窗"发布前请先补全联系方式（推荐 QQ/微信）"并跳转 Profile 编辑页。这是把后端 422 前移成一次性引导，避免提交后再被打回。
...
[选择类型]
    ...
    │   降级：直接进入手填表单                       │
    │   （每张图片对应一条草稿，                     ▼
    │    无预填字段，仅图片）            生成 N 条草稿，用户逐条审核 / 编辑
    │            │                                   │
    │            └─────────────┬─────────────────────┘
    │                          ▼
    │              批量提交（一次 HTTP 含图片 keys）
    │
    └─ 一般商品 ──→ 拍照 + LLM 提取（失败则纯手填） ──→ 表单
```

**LLM 不可用的降级**（必做分支，不属可选）：
- 用户未配置 LLM key / 配错 / 端点不通 / 模型不支持 vision 或 `response_format=json_schema` / 调用超时
- 一律回退到"纯手填表单"路径，不让任何发布流被 LLM 卡死
- 提示文案："AI 识别不可用，已切换为手动填写。可在 设置→AI 推断 检查配置。"

### 5.3 商品/求购详情页状态映射

需求 4 的"售出后免打扰"既要后端不返回联系方式，也要前端在视觉上明确告诉买家"别来打扰"。详情页据 listing/wish 状态与 seller 渲染如下：

| 状态 | 顶部状态条 | 联系入口 | 备注 |
|---|---|---|---|
| listing ACTIVE，seller 存在 | 无 | 显示卖家联系方式（QQ/微信/手机，未填的隐藏；手机号附"建议优先 QQ/微信"小字） | 正常态 |
| listing ACTIVE，seller=null | 灰色"卖家已注销" | 隐藏全部联系入口 | NapCat 来源若仍有 seller_qq 则仍可显示 QQ + 风险提示 |
| listing SOLD_OUT | 红/灰醒目"已售出" | 完全隐藏（按钮不渲染，非置灰） | 配文"该商品已售出，请勿打扰卖家" |
| listing EXPIRED | 灰"已过期" | 隐藏 | 列表中默认不展示，仅"我的发布"/直链可见 |
| listing DELISTED | 灰"已下架" | 隐藏 | 同上 |
| wish ACTIVE | 无 | 显示求购者联系方式 + 手机号风险提示 | 卖家主动联系买家入口 |
| wish FULFILLED | 灰"已找到" | 隐藏 | 配文"该求购已找到，请勿打扰" |
| wish EXPIRED | 灰"已过期" | 隐藏 |  |

ViewModel 据后端返回的 status + 联系方式是否为 null 计算 `ContactInfoUiState = Visible(...) | Hidden(reason)`，组件只读这一字段渲染，避免散落的 if/else。

### 5.4 我的发布：撤销售出

- "我的发布"列表按件展示售出状态，每件已售旁有"取消售出"按钮。
- 卖家点击"标记已售"后 5 秒内显示 Undo Snackbar；用户点 Undo 即调 `unsell` 接口，已售件回到 AVAILABLE，listing 若曾因此置 SOLD_OUT 也同步改回 ACTIVE。
- 5 秒后仍可在列表里手动取消，无时间限制。
...
6. **联系方式策略见 §4.10**：必须至少留一项才能发布商品/求购；登录后可见，且仅当 listing/wish 处于 ACTIVE 时返回（售出/下架/过期/fulfill 后即停止暴露）；手机号在客户端表单常驻风险提示，推荐 QQ/微信。NapCat 来源商品在详情页加风险提示横幅，符合"避免在平台交易"的要求。
...
**Week 1 — 地基**
- 后端：Ktor 骨架 + 结构化日志 + Postgres/MinIO 容器化 + Flyway + 用户/JWT/BIT 认证 + 上传接口 + 限流插件（先做认证接口限流，其它接口在后续迭代加）
- Android：Compose 脚手架 + 主题 + 导航 + 登录注册（含"至少一种联系方式"强制 + 手机号风险提示）+ 设置页（含 LLM 配置）+ i18n 框架
- 验收：能注册、登录、上传一张图；日志 JSON 输出正常；注册不留联系方式被拦截

**Week 2 — 核心业务**
- 后端：listings CRUD + 搜索（pg_trgm，用 `%>`）+ tags + ISBN 查询 + wishes 增删改查与求购广场浏览 + 联系方式可见性策略（仅 ACTIVE+登录）
- Android：发布流（扫码 → ISBN 预填 → 表单提交）、列表、**详情页含联系方式展示与状态→UI 映射（§5.3）**、搜索、求购广场（浏览 + 发布 + 查看求购者联系方式）
- 验收：完整一次卖书流程跑通（**含买家在详情页看到卖家联系方式**）；能发布求购、在广场看到、并看到求购者联系方式；SOLD_OUT/EXPIRED 详情页正确隐藏联系入口

**Week 3 — 智能与体验**
- Android：批量拍照 → LLM（vision，客户端直连）→ 草稿审核 → 批量提交；**LLM 不可用降级到手填表单（必做）**；筛选器；标记售出 + Undo / 我的发布"取消售出"
- 后端：批量提交接口优化；`POST .../items/{idx}/unsell`
- 验收：拍三本书一次发布成功（使用用户自己的 LLM key）；LLM 关闭/配错时仍能完成发布；误标已售可一键撤销

**Week 4 — 进阶 + 收尾**
- 后端：求购自动匹配（WISH_HIT，用 `word_similarity`/`%>`，提交后异步）+ notifications + WebSocket（wishes 基础 CRUD/广场已在 Week 2 完成）
- Android：求购匹配通知页（含"该商品已售出"落地提示）；通知页；UI 打磨；中英文核校；崩溃修复
...
- 反作弊、举报、信用分
- **发布类接口频控、详情接口反爬、联系方式批量抓取防御** — Week 1 仅做 `/auth/*` 限流；其余明知存在风险但暂不投入。等用户基数起来或观察到滥用再加（复用现有内存令牌桶；服务端需经 Caddy 透传 `X-Forwarded-For` 拿真实客户端 IP 才能按用户/IP 限流，部署时确认这一跳）
