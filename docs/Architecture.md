...
### 6.3 列表 `/listings`
- `GET /listings`：query 参数 `type, category?, q?, tagIds?, minPrice?, maxPrice?, includeNoPrice?, includeSold?, cursor?`。返回摘要列表（含 `id, type, category, title, unitPrice, quantityTotal, quantitySold, pickupLocation, nickname, firstImageUrl, tags, createdAt`；**不含** `contact`、`description` 全文），无论是否登录均可访问。
