...
- Android 的主题、语言、常用联系方式、LLM 配置和未发布草稿表单不进入服务端数据库。草稿图片可能在发布前先上传到 Blob 存储，但只有发布成功后才建立 `listing` 与 `listing_image` 记录。
- Android 使用 EAN-13 扫码取得 ISBN 候选值；服务端负责查询书籍信息，查询失败时客户端保留手工填写路径。
...
编码由应用层枚举统一维护，没有在数据库层面做校验，以增强可扩展性。
...
部分唯一索引 `app_user_active_student_id_uk(student_id) WHERE deleted_at IS NULL` 只约束有效账号，兼顾历史保留和同学号重新注册。
...
会话默认有效期由 `BITMART_SESSION_TTL_DAYS` 控制，默认值为 30 天。
...
个人通知仅允许所属用户标记已读；全员公告不维护逐用户 `read_at`，也不计入个人未读数。
...
| `user_id` | `BIGINT` | 非空 | 外键 → `app_user.id` |
| `title` | `TEXT` | 非空 | 标题；应用层要求非空白且最长 32 字符 |
...
应用层进一步校验标题、描述、数量上限、价格范围、联系方式、标签数和有效期。数据库只对最关键的数量不变量设置 `CHECK`。
...
Android 客户端单条发布最多选择 9 张图片，并在上传前将最长边压缩到 1024 px、以 JPEG 80% 质量编码；服务端上传接口默认限制单文件不超过 5 MiB。上述限制属于客户端与上传服务校验，数据库表本身不限制图片行数或文件大小。
...
ShowAPI 不可用或查询无结果时不写入缓存，注册、登录、普通发布、列表和通知等核心流程不依赖该外部服务。
...
学号唯一性、数量下界、关联唯一性、访问权限、通知已读规则等已在数据字典、完整性设计和安全设计中说明，此处不再重复。
...
- 更换图片时整体替换 `listing_image`。
- 用户注销时同时软删除其发布并撤销全部会话。
- 业务约定公告的 `user_id` 为空、个人通知的 `user_id` 非空。

以下约束由领域校验和接口配置补充：

| 对象 | 补充规则 | 实现位置 |
| --- | --- | --- |
| 发布数量 | `quantity_total` 最大为 9999；修改总量时不得低于已有 `quantity_sold` | `ListingValidator` |
| 价格 | 单价和原价可空；非空时范围为 0～99999999.99 | `ListingValidator`，上限与 `NUMERIC(10,2)` 对齐 |
| 联系方式 | 每条发布包含一个非空的联系方式 | `ListingValidator` |
| 标签 | 每条发布最多 8 个；标签去首尾空白、转小写并压缩连续空白后去重 | `ListingValidator`、`TagNormalizer` |
| 有效期 | 默认 30 天；按天数指定时范围为 1～365 天，指定绝对日期时必须晚于当前时间且不超过最大期限 | `ListingValidator` 与部署配置 |

## 规范化分析

### 第一范式

主要实体字段均为原子值。图片、标签和书籍扩展从发布主表中拆出，避免在主表中保存重复列组。联系方式因数量较少、结构可能扩展且不会参与跨发布关系查询，作为受控 JSONB 聚合保存。
...
`tag.usage_count` 是为热门标签查询维护的冗余统计值，通过牺牲规范化程度以减少开销。
...
| 索引 | 服务查询 |
| --- | --- |
| `app_user_active_student_id_uk(student_id) WHERE deleted_at IS NULL` | 有效学号查重、登录、允许注销后重注册 |
...
| `listing_active_idx(type, category, expires_at) WHERE deleted_at IS NULL` | 有效出售/求购与品类、到期过滤 |
...
通知采用同样的 `(created_at, id)` 复合游标。相较 `OFFSET`，该方式在数据持续新增和深页访问时更稳定。现有个人通知索引为 `(user_id, created_at DESC)`，公告索引为 `(created_at DESC) WHERE user_id IS NULL`，能够辅助归属过滤和时间排序。

## 事务与并发设计

| 业务操作 | 事务范围 | 并发策略 |
| --- | --- | --- |
| 注册 | 有效用户查重、创建用户、创建会话 | 部分唯一索引最终阻止同学号并发重复注册 |
...
受影响行数为 0 表示重叠数据库事务中的并发冲突，服务层抛出异常并回滚，并向客户端返回冲突结果。更新 API 没有携带客户端读取时的旧值、版本号或 ETag；因此，若另一事务已经在本请求开始前提交，服务端会以最新数据库值作为 CAS 期望值，仍可能接受源自陈旧界面的绝对新值。其他普通标量字段当前采用最后写入者覆盖策略。
...
增加 `user_id = :current_user_id`，排除已过期或删除的项；但默认不排除过期和售罄项，以便用户继续维护自己的项目。详情页复用公共列表的详情页。
...
两者按创建时间和编号统一倒序分页。未读计数统计 `user_id = 当前用户 AND read_at IS NULL`。

### 热门标签

按 `usage_count DESC` 返回高频标签。

## 数据生命周期设计

| 数据 | 删除/失效方式 | 保留策略 |
| --- | --- | --- |
| 用户 | 设置 `deleted_at` | 保留历史；有效学号唯一索引允许重注册 |
| 会话 | 设置 `revoked` 或到期失效 | 当前无自动清理任务，可后续按时间归档/清理 |
| 发布 | 设置 `deleted_at` | 公开查询排除，关联数据保留 |
| 过期/售罄发布 | 查询时过滤，不修改为删除 | 我的发布中仍可查看和维护 |
| 发布图片元数据 | 编辑时删除旧行并重建；级联删除 | 发布软删除时仍保留 |
...
3. Bearer Token 认证联合检查会话撤销、到期和用户注销。
4. 包含联系方式、完整描述和取货地点等敏感信息的详情页要求客户端处于登录状态时服务端才能返回数据。
5. 发布的项目仅允许所有者或管理员修改、删除。
6. 数据库连接密码通过部署配置提供。
7. 应用使用参数化查询/Exposed 表达式，抵抗字符串拼接 SQL 注入。
8. LLM API Key、本机常用联系方式等信息留在客户端本地保存，不上传服务器。

## 备份与恢复

当前项目作为本地课程项目，尚未部署生产级备份体系。如果未来用于实际校园服务，可以考虑：

- 每日使用 `pg_dump` 生成逻辑全量备份。
- 保留最近 7 份日备份和最近 4 份周备份。
- 定期执行恢复演练。
- 启用 PostgreSQL WAL 归档，实现时间点恢复。
- 迁移前创建可恢复备份。

## 当前设计的局限性与改进方向

- 枚举编码、价格非负、联系方式 JSON 结构和书籍类别一致性主要依赖应用层，可增加数据库 `CHECK` 约束或触发器。
- 管理员修改/删除发布的服务层授权已部分实现；管理员入口、封禁用户、发布公告等完整管理功能尚未实现。
- 标签和 ISBN 缓存的并发“先查后写”可改为原子 upsert。
- 软删除、图片替换可能留下孤立 Blob 文件，需要定时清理。
- 会话、通知、ISBN 缓存和历史发布尚无归档策略，数据规模增长后需要制定保留周期。
