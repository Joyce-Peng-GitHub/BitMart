...
学号唯一性、数量下界、关联唯一性、访问权限、通知已读规则等已在数据字典、完整性设计和安全设计中说明，此处不再重复。以下约束未完全由当前 DDL 表达，由领域校验和接口配置补充：
...
| 联系方式 | 每条发布至少包含一个联系方式，且每个联系方式的值非空 | `ListingValidator` |
...
5. 发布仅允许所有者或管理员修改、删除。
6. 数据库连接密码通过部署配置提供；示例账号口令仅用于本地课程环境。
7. 应用使用参数化查询/Exposed 表达式，避免字符串拼接 SQL 注入。
8. LLM API Key、主题、语言和本机常用联系方式不上传服务器。
9. 数据库生产部署应使用独立账号、最小权限、加密连接和受限网络访问。
...
- 枚举编码、价格非负、联系方式 JSON 结构和书籍类别一致性主要依赖应用层，可增加数据库 `CHECK` 或触发器。
